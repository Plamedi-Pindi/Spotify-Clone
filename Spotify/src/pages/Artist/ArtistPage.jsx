...
export default function ArtistPage() {
  ...
  const [isArtistActive, setIsArtistActive] = useState();
  ...
  useEffect(() => {
    const fetchArtist = async () => {
      try {
        const response = await api.get(`/artists/getartist/${id}`);
        const artist = response.data[0];
        const isnitialSong = artist.musics[0].music;

        setArtist(artist); //

        setIsArtistActive(() => {
          const item = JSON.parse(localStorage.getItem('artist'))
          return (artist.name === item.name) ? item.active : artist.active
        })

        setInitialSong(isnitialSong)  //

      } catch (error) {
        console.error("Ocorreu um erro ao buscar o artista!", error);
      }
    };
  ...
  const handlePlayClick = async () => {
    try {
      const isTheSame = currentArtist.name !== artist.name;

      if (isTheSame) {
        setCurrentArtist({ ...artist, active: true }) //
        localStorage.setItem('artist', JSON.stringify({ ...artist, active: true }))

        setIsArtistActive(true)  //
        updateCurrentSong(0, artist)
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div
          className={`flex items-center  p-2 fixed w-full z-30 ${isScrollUp ? "bg-yellow-900" : "bg-black/0"
        ...
        >
          ...
          <h2
            className={`text-lg font-medium ${isScrollUp ? "opacity-100" : "opacity-0"
        ...
        >
          <h2
            className={`text-4xl font-bold text-white mb-2 text-center ${isScrollUp ? "opacity-0" : "opacity-100"
              } duration-500 ease-in-out`}
          >
            {" "}
            {artist?.name}{" "}
          ...
          <div className="bg-gradient-to-b from-yellow-800/30 from-10% to-balck to-80% z-20">
            ...
            <div className="flex items-center justify-between p-3 pl-4 pr-4  ">
              ...
              {isArtistActive
                ? isPlaying
                  ? (
                    <BsPauseCircleFill onClick={handlePauseClick} className="text-5xl text-green-600" />
                  ) : (
                    <BsPlayCircleFill onClick={handlePlayClick} className="text-5xl text-green-600" />
                  )
                : (
                  // <div>pp</div>
                  <BsPlayCircleFill onClick={handlePlayClick} className="text-5xl text-green-600" />
        ...
        >
          <div className=" h-9p sticky top-14  bg-black pt-4 pb-2">
            <button
              className={` ${isMusic && "border-b-2 border-green-500 text-white mb-2"
            ...
            <button
              className={` ${isMerch && "border-b-2 border-green-500 text-white"
                } mr-6 text-base pb-2 text-neutral-400 `}
